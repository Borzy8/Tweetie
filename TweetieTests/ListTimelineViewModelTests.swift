...
class ListTimelineViewModelTests: XCTestCase {
  ...
  func test_whenInitialized_bindsTweets() {
    ...
    func test_whenAccountAvailable_updatesAccountStatus() {
        ...
        let account = PublishSubject<TwitterAccount.AccountStatus>()
        let viewModel = createViewModel(account.asDriver(onErrorJustReturn: .unavailable))
        
        let disposeBag = DisposeBag()
        let loggedIn = viewModel.loggedIn.asObservable().share()
        
        loggedIn.subscribe(observer)
        ...
        account.onNext(.authorized(TestData.account))
        account.onNext(.unavailable)
        account.onCompleted()
